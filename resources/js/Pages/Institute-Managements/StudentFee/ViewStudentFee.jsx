...
const ViewStudentFee = () => {
    ...
    const feesByStudent = studentFees.reduce((acc, fee) => {
                ...
                <table className="w-full border-collapse">
                    ...
                    <tbody>
                        {studentFees.length > 0 ? (
                            Object.entries(feesByStudent).map(([studentId, fees], sIndex) =>
                                fees.map((fee, index) => (
                                    <tr key={fee.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="border px-4 py-2">
                                            {fee.student?.student_name} ({fee.student?.roll_number})
                                        </td>
                                        <td className="border px-4 py-2">
                                            {fee.fee?.name} ({fee.fee?.type})
                                        </td>
                                        <td className="border px-4 py-2">৳{fee.paid_amount}</td>
                                        <td className="border px-4 py-2">
                                            {fee.months && Array.isArray(fee.months)
                                                ? fee.months.join(", ")
                                                : "-"}
                                        </td>
                                        <td className="border px-4 py-2">{fee.payment_method}</td>
                                        <td className="border px-4 py-2">
                                            {fee.payment_date
                                        ...
                                        <td className="border px-4 py-2 space-x-2">
                                            <Link
                                                href={route("student-fees.editAll", studentId)}
